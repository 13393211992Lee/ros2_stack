...
/**
 * @brief 展示了匹配事件的工作方式
 * 发布者和订阅者建立连接时，会触发匹配事件.
 * Class MatchedEventDetectNode: 输出发布者和订阅者的连接信息
 * Class MultiSubNode： 类用于创建/销毁 subscription，以连接/断开发布者的连接
 * Class MultiPubNode: 用于创建/销毁publisher 以连接/断开订阅
 */
